        ...
        <div className="bg-card/98 backdrop-blur-md border-t border-x border-border shadow-2xl max-w-4xl mx-auto rounded-t-2xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
              <ListMusic size={13} /> Daftar Lagu
            </span>
            <button onClick={() => setShowQueue(false)} className="p-1 rounded text-muted-foreground hover:text-foreground transition-colors">
              <ChevronDown size={16} />
            </button>
          </div>
            ...
            {queue.map((track, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs font-mono w-4 text-center shrink-0 text-muted-foreground">
                    {isActive && isPlaying ? (
                      <span className="flex gap-0.5 items-end h-3 justify-center">
                        {[1,2,3].map(j => (
                          <span key={j} className="w-0.5 bg-primary rounded-sm animate-bounce"
                  ...
                  {/* Album art */}
                  {track.albumArt ? (
                    <img src={track.albumArt} alt={track.title} className="w-8 h-8 rounded-md object-cover shrink-0" />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium truncate leading-tight ${isActive ? "text-primary" : ""}`}>{track.title}</p>
