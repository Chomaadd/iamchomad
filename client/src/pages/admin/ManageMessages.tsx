...
export default function ManageMessages() {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    const ok = await showConfirm({ title: t("admin.messages.confirm.delete"), description: t("admin.confirm.undone"), confirmLabel: t("admin.confirm.delete") });
    if (ok) {
      try {
        await deleteMsg(id);
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {!msg.read && (
                      <span className="px-1.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider bg-primary/15 text-primary">{t("admin.messages.badge.new")}</span>
                    )}
                  </div>
            ...
            <div className="mt-4 pt-4 border-t border-border flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                <span className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-lg font-medium ${msg.read ? "bg-muted text-muted-foreground" : "bg-primary/10 text-primary"}`}>
                  ...
                  {msg.read ? t("admin.messages.status.read") : t("admin.messages.status.unread")}
                </span>
              </div>
              <div className="flex items-center gap-2">
                {!msg.read && (
                  ...
                  >
                    <Check size={12} /> {t("admin.messages.action.markRead")}
                  </button>
                )}
                <button
                ...
                >
                  <Trash2 size={12} /> {t("admin.messages.action.delete")}
                </button>
              </div>
            </div>
          )}
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex items-end justify-between mb-8 gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-1">{t("admin.messages.label")}</p>
          <h1 className="text-2xl md:text-3xl font-serif font-bold" data-testid="text-messages-title">{t("admin.messages.title")}</h1>
        </div>
        <div className="flex items-center gap-3 text-sm text-muted-foreground shrink-0">
          {unread.length > 0 && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-primary/10 text-primary rounded-xl text-xs font-semibold">
              <MailIcon size={13} />
              {unread.length} {t("admin.messages.unread")}
            </div>
          )}
          <span className="text-xs text-muted-foreground/60">{messages?.length || 0} {t("admin.messages.total")}</span>
        </div>
      </div>
      ...
      {(!messages || messages.length === 0) && (
        <div className="flex flex-col items-center justify-center py-24 border border-dashed border-border rounded-2xl text-muted-foreground">
          ...
          <p className="font-medium text-sm">{t("admin.messages.empty.title")}</p>
          <p className="text-xs text-muted-foreground/60 mt-1">{t("admin.messages.empty.desc")}</p>
        </div>
      )}
      ...
      {unread.length > 0 && (
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{t("admin.messages.section.unread")}</span>
      ...
      {read.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{t("admin.messages.section.read")}</span>
            <div className="flex-1 h-px bg-border" />
